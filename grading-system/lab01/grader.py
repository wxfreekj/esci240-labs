...
class Lab01Grader(LabGrader):
    ...
    def __init__(self):
        ...
        self.q1_keywords = ['testable', 'natural', 'evidence', 'observable', 'consistent']
        ...
        for q_num in range(1, 7):
            ...
            if match:
                ...
                if 'TYPE: split' in q_content:
                    part_a = re.search(r'\[PART_A\](.*?)\[/PART_A\]', q_content, re.DOTALL)
                    part_b = re.search(r'\[PART_B\](.*?)\[/PART_B\]', q_content, re.DOTALL)
                    answers[f'q{q_num}a'] = part_a.group(1).strip() if part_a else ""
                    answers[f'q{q_num}b'] = part_b.group(1).strip() if part_b else ""
                else:
                    # Essay type
                    answer = re.search(r'\[ANSWER\](.*?)\[/ANSWER\]', q_content, re.DOTALL)
    ...
    def grade_submission(self, submission_file: str) -> dict:
        ...
        try:
            ...
            if 'q1a' in answers and 'q1b' in answers:
                ...
                feedback.append(f"Q1 ({q1_score:.1f}/3.0): Part A: {fb_a}, Part B: {fb_b}")
            ...
            if 'q4a' in answers and 'q4b' in answers:
                ...
                feedback.append(f"Q4 ({q4_score:.1f}/3.0): Part A: {fb_a}, Part B: {fb_b}")
            
            # Question 5: Science and Authority (4 pts)
            if 'q5' in answers:
                score, fb = self.grade_answer(answers['q5'], self.q5_keywords, 40)
                q5_score = score * 4
                total_score += q5_score
                feedback.append(f"Q5 ({q5_score:.1f}/4.0): {fb}")
            
            # Question 6: Ethical Challenges (3 pts)
            if 'q6' in answers:
                score, fb = self.grade_answer(answers['q6'], self.q6_keywords, 30)
                q6_score = score * 3
                total_score += q6_score
                feedback.append(f"Q6 ({q6_score:.1f}/3.0): {fb}")
